...
const baseApiUrl = "https://www.pandora.com/"
...
export const request = async (endpoint, options) => {
  const url = `${baseApiUrl}api/${options.version || "v1"}/${endpoint}${options.query
  ...
  if (options.content) {
    fetchOpts.body = JSON.stringify(options.content)
  }

  return await fetch(fetchOpts, (error, response, body) => {
    if (error) {
